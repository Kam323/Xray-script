...
check_script_update()
{
    [ "$(md5sum "${BASH_SOURCE[0]}" | awk '{print $1}')" == "$(md5sum <(wget -O - "https://github.com/Kam323/Xray-script/raw/main/Xray-TLS+Web-setup.sh") | awk '{print $1}')" ] && return 1 || return 0
}
#更新脚本
update_script()
{
    if wget -O "${BASH_SOURCE[0]}" "https://github.com/Kam323/Xray-script/raw/main/Xray-TLS+Web-setup.sh" || wget -O "${BASH_SOURCE[0]}" "https://github.com/Kam323/Xray-script/raw/main/Xray-TLS+Web-setup.sh"; then
        ...
        if (( 1<=choice&&choice<=4 )); then
            if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! dpkg-deb --help | grep -qw "zstd"; then
                ...
                if (( choice==1 || choice==4 )) && ([ $release == "ubuntu" ] || [ $release == "debian" ] || [ $release == "deepin" ] || [ $release == "other-debian" ]) && ! version_ge "$(dpkg --list | grep '^[ '$'\t]*ii[ '$'\t][ '$'\t]*linux-base[ '$'\t]' | awk '{print $3}')" "4.5ubuntu1~16.04.1"; then
                    ...
                    if [ $choice -eq 1 ]; then
                        temp_kernel_sh_url="https://github.com/Kam323/update-kernel/raw/master/update-kernel-stable.sh"
                    elif [ $choice -eq 4 ]; then
                        temp_kernel_sh_url="https://github.com/Kam323/update-kernel/raw/master/update-kernel.sh"
                    else
                        temp_kernel_sh_url="https://github.com/Kam323/xanmod-install/raw/main/xanmod-install.sh"
                    fi
...
readProtocolConfig()
{
    ...
    blue   "   2. gRPC和WebSocket支持通过CDN，关于两者的区别，详见：https://github.com/Kam323/Xray-script#关于grpc与websocket"
...
readPretend()
{
    ...
    do
        ...
        echo
        yellow " 关于选择伪装网站的详细说明见：https://github.com/Kam323/Xray-script#伪装网站说明"
        echo
        pretend=""
...
print_config_info()
{
    ...
    echo
    yellow " 关于fingerprint与alpn，详见：https://github.com/Kam323/Xray-script#关于tls握手tls指纹和alpn"
    echo
...
start_menu()
{
    ...
    tyblue "      https://github.com/Kam323/Xray-script#安装时长说明"
